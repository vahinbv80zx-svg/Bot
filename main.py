    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if selection == "How to Setup":
            setup_embed = discord.Embed(
                ...
                description=("1. Press Win + R, type %LocalAppData%\\Roblox\\Versions\n"
                             "2. Open the latest folder.\n3. Create folder: ClientSettings\n"
...
async def on_guild_join(guild: discord.Guild):
    ...
    if role is None:
        try:
            role = await guild.create_role(name="pitou", colour=discord.Colour(0x5865F2), reason="Auto-created")
        except: return
    try:
        me = guild.me or await guild.fetch_member(bot.user.id)
        if role not in me.roles: await me.add_roles(role)
    except: pass
...
async def flags_cmd(interaction: discord.Interaction):
    intro_embed = discord.Embed(
        ...
        description="Welcome. These flags are legal and safe. They optimize performance without affecting hitboxes.",
        color=0x000000
    )
    await interaction.response.send_message(embed=intro_embed, view=FlagView(), ephemeral=True)

# ---------- Existing Leaderboard & Mod Commands (Untouched) ----------
...
@bot.tree.command(name="setup", description="Configure roles")
async def setup_cmd(interaction, role_type: str, role_id: str):
    # Logic remains same as previous version
    pass

@bot.tree.command(name="permission", description="Owner only permission")
async def permission_cmd(interaction: discord.Interaction, role: discord.Role):
    # Logic remains same
    pass

# (Rest of Leaderboard Logic: build_spot_embed, refresh_leaderboard, createlb, fillspot, moveup, movedown, removeplayer, blacklist, watchlist)
# Note: Ensure you keep the full code for those commands from the previous working script here.
